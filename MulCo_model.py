...
class BertTemporalOrdering(nn.Module):
    def __init__(self, encoder, config, use_bilstm=False, bilstm_hidden_size=128, bilstm_dropout=0.1):
        super(BertTemporalOrdering, self).__init__()
        self.encoder = encoder
        self.config = config
        self.use_bilstm = use_bilstm
        self.bilstm_hidden_size = bilstm_hidden_size
        self.bilstm_output_size = 6 * bilstm_hidden_size if use_bilstm else 0
        ...
        if self.use_bilstm:
            ...
            self.bilstm_dropout = nn.Dropout(bilstm_dropout)
        else:
            self.segment_bilstm = None
            self.bilstm_dropout = None
    ...
    def forward(self, batch):
        ...
        bilstm_reps = None
        if self.use_bilstm:
            segment_seq = torch.stack(sub_reps, dim=1)
            lstm_out, _ = self.segment_bilstm(segment_seq)
            e1_seq_rep = lstm_out[:, 1, :]
            middle_seq_rep = lstm_out[:, 2, :]
            e2_seq_rep = lstm_out[:, 3, :]
            bilstm_reps = torch.cat([e1_seq_rep, middle_seq_rep, e2_seq_rep], dim=-1)
            bilstm_reps = self.bilstm_dropout(bilstm_reps)

        sub_reps.append(full_emb)
        final_reps = torch.cat(sub_reps, dim=1)

        return final_reps, full_emb, bilstm_reps
...
class MulCo(nn.Module):
    def __init__(self, syntax_gcn, temporal_gcn, context_encoder, classifier, dropout, k_hops, temp=1,
                 ...
                 gamma_inv=0.1, label_reverse_map=None):
    ...
    def forward(self, batch):
        ...
        if self.context_encoder:
            context_outputs = self.context_encoder((tokenized_inputs, indices))
            if len(context_outputs) == 3:
                bert_reps, bert_embs, bilstm_reps = context_outputs
            else:
                bert_reps, bert_embs = context_outputs
                bilstm_reps = None
            concat_reps.append(bert_reps)
            bert_outputs = self.bert_classifier(bert_reps)
            if bilstm_reps is not None:
                concat_reps.append(bilstm_reps)
